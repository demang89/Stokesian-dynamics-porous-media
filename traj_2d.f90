...
real(8) :: r_frac(2)
integer :: r_grid(2)
real(8) :: ract(2), rtemp(2), rij(2)
real(8) :: var(2), Fr(2), Ft(2), vinf(2), Ff(2), Fc(2)
real(8) :: rself(2,2), rsum(2,2), r11(2,2), r12(2,2), umat(2,2)
real(8) :: lt(2,2), lt_temp(2,2), mij(2,2), mij_temp(2,2)
real(8) :: test(4)
...
read(14,*)
read(14,*) nmax, dt !No. of timesteps and timestep value
read(14,*)
...
!Identity matrix
umat=0.d0 
forall(i=1:2) umat(i,i)=rp

rtot=rp+rnp; rtot2=rtot*rtot
const=dsqrt(2.0d0/dt); mc=200.0d0
vinf=(/dble(vmax)*dcos(Pi/180.d0*dble(theta)),dble(vmax)*dsin(Pi/180.d0*dble(theta))/)


allocate(rsout(0:ngrid(1), 0:ngrid(2), dm, dm)) !Allocate array for Rsum matrix
allocate( rout(0:ngrid(1), 0:ngrid(2), dm, dm)) !Allocate array for Rself matrix
...
do i=0,ngrid(1)
        do j=0,ngrid(2)
                ...
                rout(i,j,1:dm,1:dm)=reshape(test,(/dm,dm/))
                if(reason > 0) then
                ...
                rsout(i,j,1:dm,1:dm)=reshape(test,(/dm,dm/))
                if(reason > 0) then
...
do while(dhs2 .eq. -1.d0)
        ...
        ract=(/dble(x1)*cell_size(1),dble(x2)*cell_size(2)/)
        call periodic_HSInteraction(ract,rtot2,dhs2)
end do
write(14,1000) 0, ract(1), ract(2)
...
if(vmax .eq. 0.d0) then
        ...
        if(longrange .and. shortrange) then
                do p=1,nmax
                        ...
                        call bilinearinterpolation(r_grid,r_frac,rout,rself)
                        do j=1,natom-1
 ...
 !If there is external flow
        ...
        if(longrange .and. shortrange) then
                do p=1,nmax
                        call grid_location(ract,r_grid,r_frac)
                        call bilinearinterpolation(r_grid,r_frac,rout,rself)
                        call bilinearinterpolation(r_grid,r_frac,rsout,rsum)
                        do j=1,natom-1
                                rij=ract-rpos(:,j); call pbc(rij); dr2=sum(rij*rij)
                                if(dr2<50.0d0)then
                                        call lub11correction(rij, r11, rp, rnp)
...
1000 format(i11,x,f20.8,x,f20.8)
close(14)
